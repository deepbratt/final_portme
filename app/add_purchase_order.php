<?php
 ...
 $last_purchase_id = mysqli_insert_id($mysqli);

	if($insert_purchase_order)
		{	
			$get_quantity= mysqli_query($mysqli, "select * from tbl_transactions where tbl_transaction_id = '".$last_purchase_id."'");
			$fetch_quantity_info = mysqli_fetch_array($get_quantity);
			$product_id = explode(",",$fetch_quantity_info['product_id_array']);								
			...
			echo "<script>window.location.href='print_invoice_purchase.php?order_id=$last_purchase_id'</script>";	
		}
		else
		{
			$data = "error";
                  ...
                  <div class="panel-body">
                    <div class="col-md-7 col-sm-12" style="padding:0px;margin:0px;">
						...
						<div class="row">
                          ...
                          <div class="col-sm-6">
                            <div class="form-group">
                              <input  name="date_invoice" type="text" class="form-control rs-datepicker" data-date-format="dd-mm-yyyy" value=<?php echo date('d/m/Y')?> readonly>
							  <p class="help-block with-errors"></p>
                            </div>
                          </div>
                        </div>
